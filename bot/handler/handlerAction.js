...
module.exports = (api, threadModel, userModel, dashBoardModel, globalModel, usersData, threadsData, dashBoardData, globalData) => {
	...
	return async function (event) {
		...
		switch (event.type) {
			...
			case "message_reply":
			case "message_unsend": {
				// Check noPrefixMode logic
				const threadData = await threadsData.get(event.threadID);
				const noPrefixMode = threadData?.data?.noPrefixMode === true;
				const prefix = global.config.PREFIX || "!";
				const body = event.body || "";

				if (noPrefixMode && !body.startsWith(prefix)) {
					const commandName = body.split(" ")[0].toLowerCase();
					const command = global.client.commands.get(commandName) || global.client.aliases.get(commandName);
					if (!command) return; // Ignore random text when noPrefix is on
				}
