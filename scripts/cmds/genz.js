...
module.exports = {
  ...
  onStart: async function ({ message, args, event, usersData }) {
    ...
    if (!prompt) return message.reply("❗ Please provide a prompt or reply to a message to use as the prompt.");
    ...
    if (data[uid].count >= 10) {
      return message.reply(`⚠️ You have reached your daily image generation limit of 10. Try again tomorrow.`);
    }

    const tagUser = {
      body: `⏳ Dear ${name}, crafting your visual masterpiece from your imagination...`,
    ...
    message.reply(tagUser, async (err, info) => {
      try {
        ...
        message.reply({
          body: `🖼️ Image generated successfully in ${timeTaken}s.`,
          attachment: stream
        }, () => {
        ...
        message.reply("❌ An error occurred while generating your image. Please try again later.");
      }
    });
  }
};
